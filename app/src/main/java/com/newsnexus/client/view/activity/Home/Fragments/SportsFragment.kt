...
class SportsFragment : Fragment(), HomeCategoriesCommunicator {
    ...
    private fun setUpView(){
        ...
        val appPreferences = this@SportsFragment.requireActivity().getSharedPreferences(Constants.PREFERENCES.APP_PREFERENCES, Context.MODE_PRIVATE)
        ...
        homeViewModel.isFail.observe(this@SportsFragment.requireActivity(), {
            if(it){
                ...
                this@SportsFragment.requireActivity().showPopupNotification(
    ...
    override fun searchOnSelectedCategories(inputString: String) {
        ...
        homeViewModel.newsCollection2.observe(this@SportsFragment, {collectionNews ->
            ...
            if(!collectionNews.articles.isNullOrEmpty()){
                ...
                Toast.makeText(this@SportsFragment.requireActivity(), "Shows no result", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    override fun clearSearching() {
        ...
        homeViewModel.newsCollection2.observe(this@SportsFragment.requireActivity(), {collectionNews ->
            if(!collectionNews.articles.isNullOrEmpty()){
                itemNewsAdapter!!.updateItem(collectionNews.articles!!)
            }else{
                Toast.makeText(this@SportsFragment.requireActivity(), "Couldn't fetch data", Toast.LENGTH_SHORT).show()
            }
        })
    }
}